...
def ensure_sqlite_schema():
    ...
    required_columns = {
        ...
        "selected_goal_type": "ALTER TABLE users ADD COLUMN selected_goal_type VARCHAR DEFAULT 'lose'",
    ...
    with engine.begin() as connection:
        ...
        quest_required_columns = {
            ...
            "domain": "ALTER TABLE quests ADD COLUMN domain VARCHAR",
            "quest_code": "ALTER TABLE quests ADD COLUMN quest_code VARCHAR",
            "phase_code": "ALTER TABLE quests ADD COLUMN phase_code VARCHAR",
            "is_required": "ALTER TABLE quests ADD COLUMN is_required BOOLEAN DEFAULT 0",
            "is_repeatable": "ALTER TABLE quests ADD COLUMN is_repeatable BOOLEAN DEFAULT 0",
            "quest_state": "ALTER TABLE quests ADD COLUMN quest_state VARCHAR DEFAULT 'available'",
            "reason_text": "ALTER TABLE quests ADD COLUMN reason_text TEXT",
            "payload_json": "ALTER TABLE quests ADD COLUMN payload_json TEXT",
        ...
        index_ddls = [
            ...
            "CREATE INDEX IF NOT EXISTS ix_quests_domain_state_code ON quests (domain, quest_state, quest_code)",
            "CREATE INDEX IF NOT EXISTS ix_quests_user_domain_created ON quests (user_id, domain, created_at)",
            "CREATE INDEX IF NOT EXISTS ix_user_health_profiles_goal_type ON user_health_profiles (goal_type)",
            "CREATE UNIQUE INDEX IF NOT EXISTS uq_weight_entries_user_date_source ON weight_entries (user_id, date_local, source)",
            "CREATE INDEX IF NOT EXISTS ix_step_days_user_updated ON step_days (user_id, updated_at)",
            "CREATE INDEX IF NOT EXISTS ix_weekly_reviews_completed ON weekly_reviews (user_id, completed_at)",
            "CREATE INDEX IF NOT EXISTS ix_walking_plan_states_mode ON walking_plan_states (current_mode)",
            "CREATE INDEX IF NOT EXISTS ix_motivation_states_band ON motivation_states (motivation_band)",
        ]

...
class User(Base):
    ...
    selected_goal_type = Column(String, default="lose", index=True)
    ...
    health_profile = relationship("UserHealthProfile", back_populates="user", cascade="all, delete-orphan", uselist=False)
    weight_baseline = relationship("WeightBaseline", back_populates="user", cascade="all, delete-orphan", uselist=False)
    weight_entries = relationship("WeightEntry", back_populates="user", cascade="all, delete-orphan")
    step_days = relationship("StepDay", back_populates="user", cascade="all, delete-orphan")
    walking_plan_state = relationship("WalkingPlanState", back_populates="user", cascade="all, delete-orphan", uselist=False)
    weekly_reviews = relationship("WeeklyReview", back_populates="user", cascade="all, delete-orphan")
    motivation_state = relationship("MotivationState", back_populates="user", cascade="all, delete-orphan", uselist=False)
